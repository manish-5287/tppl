...
export class Indent extends Component {
  ...
  handleIndent = async () => {
    try {
      ...
      const response = await makeRequest(BASE_URL + '/mobile/indent',params);
      const {success, message, indentDetails} = response;

      if (success) {
        const modificationIndentDetails = indentDetails.map(
          ({
            indent_id,
            contact_name,
            product,
                  ...
                  <Row
                    ...
                    data={[
                      <TouchableOpacity
                        key="indent_id"
                        onPress={() =>
                          this.handlePressProductID(rowData.indent_id)
                        }>
